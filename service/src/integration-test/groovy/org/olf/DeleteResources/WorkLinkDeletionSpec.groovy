...
class WorkLinkDeletionSpec extends DeletionBaseSpec{
  ...
  def setup() {
    ...
    importPackageFromFileViaService('hierarchicalDeletion/work_link_deletion.json')

    resp = doGet("/erm/packages", [filters: ['name==K-Int Link - Deletion Test Package 001']])
    resp2 = doGet("/erm/packages", [filters: ['name==K-Int Work Link - Deletion Test Package 001']])
  ...
  void "Load Packages"() {
    ...
    List resp2 = doGet("/erm/packages", [filters: ['name==K-Int Work Link - Deletion Test Package 001']])
