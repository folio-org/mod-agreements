...
public class GOKbOAIAdapter extends WebSourceAdapter implements KBCacheUpdater, DataBinder {
  ...
  private InternalPackageImpl gokbToERM(GPathResult xml_gokb_record, boolean trustedSourceTI) {
    ...
    if ( ( package_record != null ) && ( package_record.name != null ) ) {
      ...
      List<String> alternate_resource_names = package_record.variantNames?.findAll{it.variantName?.text() != null && it.variantName?.text()?.trim() != ''}?.collect{ it.variantName?.text()}
      def arns = alternate_resource_names.size()  // TODO: remove
      println("a_r_n: ${alternate_resource_names}, ${arns}")  // TODO: remove
