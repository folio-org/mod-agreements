...
| `EHOLDINGS_SYNC_BUFFER`         | Minimum age of the most recent `EHoldingsEntitlementSyncJob` before the hourly `_timer` will queue another. Throttles the effective cadence — short to fire each tick, long to disable. Manual `GET /erm/admin/triggerEntitlementEholdings?force=true` bypasses this buffer.         | <ul><li>`ONE_DAY`</li><li>`ZERO`</li><li>Any integer -- representing milliseconds</li></ul>                                                          | `24*60*60*1000` (`ONE_DAY`)   |
